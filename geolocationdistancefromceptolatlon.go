...
// GeolocationDistanceFromCepToLatlonService contains methods and other services
// that help with interacting with the findcep API.
//
// Note, unlike clients, this service does not read variables from the environment
// automatically. You should not instantiate this service directly, and instead use
// the [NewGeolocationDistanceFromCepToLatlonService] method instead.
type GeolocationDistanceFromCepToLatlonService struct {
	Options []option.RequestOption
}

// NewGeolocationDistanceFromCepToLatlonService generates a new service that
...
func NewGeolocationDistanceFromCepToLatlonService(opts ...option.RequestOption) (r *GeolocationDistanceFromCepToLatlonService) {
	r = &GeolocationDistanceFromCepToLatlonService{}
	r.Options = opts
	return
}
...
func (r *GeolocationDistanceFromCepToLatlonService) List(ctx context.Context, cepFrom string, latlonTo string, opts ...option.RequestOption) (res *GeolocationCep, err error) {
	...
	path := fmt.Sprintf("v1/geolocation/distance/from/cep/%s/to/latlon/%s", cepFrom, latlonTo)
